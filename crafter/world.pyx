...
class World:
    def __init__(self):
        ...
        self.group: TextureGroup = (
            TextureGroup(image.load(TEXTURE_PATH).get_texture())
        )
    ...
    def show_block(self, position, immediate=True):
        ...
        This method assumes the block has already been added with add_block()
    ...
    def process_queue(self):
        ...
        This allows the game loop to run smoothly.
        The queue contains calls to _show_block() and _hide_block()
        so this method should be called if add_block() or remove_block()
        was called with immediate=False
        """
        start = time.monotonic()
        while self.queue and time.monotonic() - start < 1.0 / TICKS_PER_SEC:
            self._dequeue()
